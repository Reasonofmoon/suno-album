# Gemini 3.0 Pro × Suno v5 Symphonic Storm 앨범 워크플로우 (v1.1)

이 폴더는 “앨범 단위(약 15곡)”로 **Suno v5 장르 프롬프트 + 가사**를 뽑고, 검토/재시도 후 **단일 MD 파일**로 정리하는 대화형 워크플로우다. 모든 곡은 Symphonic Storm DNA(220–240 BPM, micro hook, Verse 2 chopper flip)를 기준으로 만든다. 마지막에 **Gemini 내장 이미지 생성**으로 앨범 커버를 만들기 위한 프롬프트까지 포함한다.
...
- `reports/Symphonic_Storm_Composition_Report_KR.md`: 참고용 분석 요약 보고서(기술/구조 관점)

## 이 저장소의 기존 자료(레거시 참고)
- `Achmage_Symphony Hiphop_Rap Agent_1.3.txt`: 1곡 단위 생성용 기준 프롬프트
- `BewhY Lyric Analysis Framework.md`: 레거시 분석 보고서(영문)
- `Achmage_BewhY_Rap_Agent_1.4.txt`: 레거시 1곡 프롬프트
...
## 가사 글자 예산 체크
- 섹션별 예산(±10%)과 총합 1,300~2,200자 기준은 `OUTPUT_SCHEMA.md` 참고.
