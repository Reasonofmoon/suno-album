...
# Load Track Data
DATA_FILE = os.path.join(os.path.dirname(__file__), "album3_data.json")
...
def main():
    ...
    print(f"🚀 Starting Production for Album 3: Gumaetan Market Vol. 3 ({len(tracks)} tracks)")
    ...
    for i, track in enumerate(tracks):
        ...
        if not run_command(f'python src/publish.py {task_id} album_3'):
            print("❌ Publishing failed.")
        else:
            print("💾 Committing to Git...")
            run_command(f'git add . && git commit -m "Feat: Add Album 3 Track {title}" && git push origin main')
